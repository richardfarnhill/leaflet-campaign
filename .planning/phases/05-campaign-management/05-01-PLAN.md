...
<task type="auto">
  ...
  <action>
    **Context:** Finance projections already use real enquiry/case data when available
    (avgCV = real avg if data exists, else DEFAULT_CV £294.42). Baseline rates (0.25%/0.5%/1.0%)
    are hardcoded but used as fallback correctly. This task stores these per campaign.

    Add to campaign config modal:
    ```html
    <div class="modal-field">
      <label>Response Rate Scenarios</label>
    ...
    Also add a default case value input (CFG-04):
    ```html
    <div class="modal-field">
      <label>Default Case Value (£)</label>
      <input type="number" step="0.01" id="configDefaultCaseValue" placeholder="294.42">
    </div>
    ```

    Store in campaign table (add columns: rate_conservative, rate_target, rate_optimistic,
    default_case_value). Update finance projections to use campaign values with hardcoded
    defaults as fallback when columns are null.
  </action>
  <verify>Custom response rates affect revenue projections</verify>
  <done>Response rates configurable per campaign</done>
</task>
...
- [ ] All Campaigns view shows aggregated totals
- [ ] Response rates configurable per campaign
